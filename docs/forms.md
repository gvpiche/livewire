...
Now if the `$title` property is updated before the form is submitted—like when using [`wire:model.blur.live`](/docs/4.x/wire-model#updating-on-blur-event)—the validation for `$title` will be run.
...
<form wire:submit>
    ...
    <input type="text" wire:model.blur.live="content">
    <div>
...
```blade
<input type="text" wire:model.blur.live="title" wire:dirty.class="border-yellow">
```
